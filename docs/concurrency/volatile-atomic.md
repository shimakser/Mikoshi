...
Модификатор вынуждает потоки отключить оптимизацию доступа и использовать единственный экземпляр переменной.
...
volatile даёт видимость и запрет некоторых reorderings вокруг volatile-операций. Запись в volatile публикует предшествующие изменения, а последующее чтение volatile в другом потоке их увидит. Это делает volatile хорошим решением для флагов остановки, публикации уже сконструированного immutable-объекта и простых состояний.

**Но volatile не даёт взаимного исключения — eсли два потока делают counter++ на volatile-поле, потеря обновлений возможна**, потому что операция состоит из чтения, инкремента и записи.

**synchronized даёт больше:**
- mutual exclusion;
- visibility на входе и выходе из критической секции;
- happens-before между unlock и последующим lock того же монитора.
