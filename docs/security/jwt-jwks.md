...
Если мы используем RS256, возникает вопрос: как микросервисы (Resource Servers) узнают публичный ключ для проверки токена?

- JWKS (JSON Web Key Set): Auth Server выставляет публичный эндпоинт (обычно /.well-known/jwks.json), который возвращает список публичных ключей в формате JSON.
  - Key Rotation (Ротация ключей): Из соображений безопасности ключи нужно периодически менять. Но если просто заменить ключ, все ранее выданные (но еще не протухшие) токены станут невалидными.
    Как это решается с помощью kid (Key ID):
      - Auth Server имеет несколько активных пар ключей.
      - При создании токена в его Header добавляется поле "kid": "key-2026-05".
      - Микросервис получает токен, видит kid и ищет ключ с таким же kid в списке, полученном из JWKS эндпоинта.
      - Для ротации Auth Server просто генерирует новый ключ с новым kid, начинает подписывать новые токены им, но оставляет старый публичный ключ в JWKS, пока не протухнут старые токены.