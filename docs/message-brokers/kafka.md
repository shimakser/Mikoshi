...
## Батчинг в Kafka-продюсере

Продюсер не отправляет каждое сообщение сразу как своё send(), а пытается собрать несколько сообщений в один “батч” (batch), чтобы отправить их одной сетевой операцией. Это уменьшает накладные расходы на сетевые вызовы и увеличивает пропускную способность.

Батч отправляется в двух случаях:
1. Когда накопленный размер батча (`batch.size`) достигнут.
2. Когда прошло время ожидания (`linger.ms`), даже если батч не полный.

Также батч может быть отправлен принудительно, если продюсер закрывается или flush вызывается.

### Параметры конфигурации

- `batch.size` — максимальный размер батча в байтах.
- `linger.ms` — сколько миллисекунд продюсер готов “ждать”, прежде чем отправить неполный батч.
- `buffer.memory` — сколько буфера (памяти) продюсер выделяет под накопление сообщений.

---

## Compression

Бывает выгодно не отправлять батч “сырой”, а сжать весь батч перед отправкой. Это уменьшает объём передаваемых данных по сети и, возможно, уменьшает нагрузку на хранение на брокере.

При чтении consumer’ом, клиентская библиотека сам распаковывает батч.

### Типы компрессии
Поддерживаются несколько алгоритмов: 
- none,
- gzip, 
- snappy, 
- lz4, 
