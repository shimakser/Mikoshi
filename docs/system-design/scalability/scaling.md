...
- Нужно Отделять "масштабирование" от "ускорения" — иногда нужно оптимизировать код, а не добавлять узлы.
- Сначала находим bottleneck (CPU, IO, База, Сеть).
- Делим на вертикальное и горизонтальное.
- Stateless-first дизайн.
- Планировать auto-scaling.
- Никаких unbounded очередей.
- Масштабировать независимые компоненты отдельно