...
Балансировка без привязки клиента к конкретному инстансу (no session stickiness). LB/Ingress распределяет запросы независимо.

**Почему это полезно:**
* упрощает autoscaling и rolling deploys;
* избегает hotspots/неравномерной загрузки;
* совместимо со stateless APIs.
  
**Когда не использовать:** если приложение по дизайну нуждается в локальном состоянии (плохой дизайн).

**Как решают проблему stateful-потребностей без sticky:**
* persist state в DB/Cache,
* use tokens (JWT) или client-side state,
* server-side state replicated in a distributed cache (Hazelcast/Redis).