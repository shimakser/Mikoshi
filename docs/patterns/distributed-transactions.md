...
**Idempotent processing** — выполнение операции N раз даёт тот же результат, что и 1 раз. Важно при ненадёжных сетях и retries.

_Как достичь:_
* Idempotency key: клиент посылает idempotency-id; сервер сохраняет результат (dedup store) и возвращает тот же ответ при повторе.
* Определять операции, которые можно повторять без побочных эффектов (например, PUT вместо POST).
* Использовать at-least-once delivery + dedup: consumers применяют dedup по ключу события.
* Опирайтесь на natural id (order_id) — если есть уникальный ид ресурса, попытка повторного создания фиксируется как duplicate.

Где хранить dedup state:
- short-lived in-memory cache + persistent store for longer (Redis with TTL + DB for finality).